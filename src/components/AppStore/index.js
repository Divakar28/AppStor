...
class Home extends Component {
  state = {userInput: '', activeId: tabsList[0].tabId}
  ...
  getSearchResults = () => {
    ...
    const filterValues = appsList.filter(each =>
  ...
  render() {
    const {userInput, activeId} = this.state
    ...
    return (
      <div className="bg">
        ...
        <ul className="tab-lists">
          {tabsList.map(each => (
            <TabItem key={each.tabId} tabItem={each} />
          ))}
        </ul>
        <ul className="app-lists-item">
